...
TESTROOT = os.getenv('portfoliotestroot')
extra_templates = {}
        ...
        try:
            if pagemap.get('expanded', None):
                ...
                with open(TESTROOT+
                    ...
                    extra_templates[filename+'.html'] = rendered_page
                return pagemap
...
def main():
    ...
    JINJA_CONSTANTS['TEMPLATE_URL'] = args.template
    JINJA_CONSTANTS['SITE_URL'] = 'http://josephhader.com' if args.upload\
                                  else 'http://localhost:8000'
    if args.template != "base.html":
        post_content = get_posts(args.template.split(".")[0])
    else:
        post_content = None

    rendered_page = unicode(render_template(args.template,
    ...
    if args.upload:
        ...
        upload_template(bucket, args.template, rendered_page)

        for template in extra_templates:
            upload_template(bucket, template, extra_templates[template])

    with open(TESTROOT+"output/"+args.template, 'w') as output_file:
